...
import { employeeDocumentFilename } from '../../shared/file-names.js'
import { cleanRut, isValidRut } from '../../shared/rut.js'
...
export function RrhhDocumentos() {
  ...
  const selectedPerson = people.find((person) => person.id === documentForm.persona_id) || null
  const selectedType = types.find((type) => type.id === documentForm.tipo_documento_id) || null
  const employeeFilePreview = useMemo(() => {
    if (!file || !selectedPerson?.rut || !selectedType) return ''
    try {
      const extension = file.name.includes('.') ? file.name.split('.').pop() : ''
      return employeeDocumentFilename({ rut: selectedPerson.rut, type: selectedType.nombre, issueDate: documentForm.fecha_emision, extension })
    } catch {
      return ''
    }
  }, [documentForm.fecha_emision, file, selectedPerson, selectedType])
  ...
  async function uploadDocument() {
    ...
    const person = people.find((item) => item.id === documentForm.persona_id)
    const type = types.find((item) => item.id === documentForm.tipo_documento_id)
    if (!person?.rut || !isValidRut(person.rut)) throw new Error('El trabajador debe tener un RUT válido antes de subir documentos.')
    if (!type) throw new Error('Selecciona el tipo de documento antes de subir el archivo.')
    const extension = file.name.includes('.') ? file.name.split('.').pop() : ''
    const fileName = employeeDocumentFilename({ rut: person.rut, type: type.nombre, issueDate: documentForm.fecha_emision, extension })
    const path = `${activeEmpresaId}/${cleanRut(person.rut)}/${fileName}`
  ...
  async function saveDocument() {
    ...
    if (!documentForm.tipo_documento_id) return setMessage('Selecciona qué tipo de documento estás subiendo.')
    if (file && (!selectedPerson?.rut || !isValidRut(selectedPerson.rut))) return setMessage('La ficha del trabajador debe tener un RUT válido antes de subir archivos.')
  ...
  return <div className="space-y-5">
    ...
    {showDocumentForm && <Card><div className="mb-5 flex items-start justify-between"><div><h3 className="text-xl font-black">Agregar documento</h3><p className="mt-1 text-sm text-slate-500">Selecciona qué documento es. Los archivos quedan privados en /var/www/desarrollo/doc/empleado y se nombran por RUT.</p></div><button onClick={() => setShowDocumentForm(false)} className="rounded-lg bg-slate-100 p-2"><X size={18} /></button></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><label className={labelClass}>Trabajador *<select value={documentForm.persona_id} onChange={(e) => setDocumentForm({ ...documentForm, persona_id: e.target.value })} className={inputClass}><option value="">Seleccionar</option>{people.map((person) => <option key={person.id} value={person.id}>{person.nombre} · {person.rut || 'sin RUT'}</option>)}</select>{selectedPerson && (!selectedPerson.rut || !isValidRut(selectedPerson.rut)) && <span className="mt-1 block text-xs font-bold text-red-700">Esta ficha necesita un RUT válido para recibir archivos.</span>}</label><label className={labelClass}>¿Qué documento estás subiendo? *<select value={documentForm.tipo_documento_id} onChange={(e) => selectType(e.target.value)} className={inputClass}><option value="">Seleccionar tipo</option>{types.filter((type) => type.activo).map((type) => <option key={type.id} value={type.id}>{type.nombre}{type.obligatorio ? ' · obligatorio' : ''}</option>)}</select></label><label className={`${labelClass} md:col-span-2`}>Nombre visible *<input value={documentForm.nombre} onChange={(e) => setDocumentForm({ ...documentForm, nombre: e.target.value })} className={inputClass} /></label><label className={labelClass}>Emisión<input type="date" value={documentForm.fecha_emision} onChange={(e) => setDocumentForm({ ...documentForm, fecha_emision: e.target.value })} className={inputClass} /></label><label className={labelClass}>Vencimiento<input type="date" value={documentForm.fecha_vencimiento} onChange={(e) => setDocumentForm({ ...documentForm, fecha_vencimiento: e.target.value })} className={inputClass} /></label><label className={labelClass}>Estado<select value={documentForm.estado} onChange={(e) => setDocumentForm({ ...documentForm, estado: e.target.value })} className={inputClass}><option value="pendiente">Pendiente</option><option value="vigente">Vigente</option><option value="vencido">Vencido</option><option value="rechazado">Rechazado</option><option value="archivado">Archivado</option></select></label><label className={labelClass}>Archivo privado<input type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.xlsx" onChange={(e) => setFile(e.target.files?.[0] || null)} className={`${inputClass} text-sm`} /></label><label className={`${labelClass} md:col-span-2`}>O URL externa<input type="url" value={documentForm.url} onChange={(e) => setDocumentForm({ ...documentForm, url: e.target.value })} className={inputClass} placeholder="https://..." /></label><label className={`${labelClass} md:col-span-2`}>Notas<input value={documentForm.notas} onChange={(e) => setDocumentForm({ ...documentForm, notas: e.target.value })} className={inputClass} /></label></div>{file && <div className="mt-3 rounded-lg bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-800"><p>Archivo seleccionado: {file.name} · {(file.size / 1024 / 1024).toFixed(2)} MB</p>{employeeFilePreview && <p className="mt-1 break-all text-xs">Se guardará como: {employeeFilePreview}</p>}</div>}<button onClick={saveDocument} disabled={saving} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-blue-700 px-5 py-3 font-black text-white disabled:opacity-50"><FilePlus2 size={17} />{saving ? 'Guardando documento...' : 'Guardar documento'}</button></Card>}
