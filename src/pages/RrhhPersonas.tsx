...
export function RrhhPersonas() {
  ...
  function editPerson(person: PersonaRrhh) {
    ...
    setForm({
      tipo_relacion: person.tipo_relacion || 'contrato', rut: person.rut || '', nombre: person.nombre || '', email: person.email || '', telefono: person.telefono || '',
      ...
      fecha_ingreso: person.fecha_ingreso || '', fecha_termino: person.fecha_termino || '', estado_laboral: person.estado_laboral || 'activo', tipo_contrato: person.tipo_contrato || 'indefinido',
    ...
    {showForm && <Card>
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <section className="space-y-3 rounded-2xl border border-slate-200 p-4"><h4 className="font-black text-slate-900">Identificación y contacto</h4>
          ...
          <div className="grid gap-3 sm:grid-cols-2"><label className={labelClass}>RUT<input value={form.rut} onChange={(e) => update('rut', e.target.value)} className={inputClass} placeholder="12.345.678-9" /></label><label className={labelClass}>Código interno<input value={form.codigo_empleado} onChange={(e) => update('codigo_empleado', e.target.value)} className={inputClass} placeholder="EMP-001" /></label></div>
