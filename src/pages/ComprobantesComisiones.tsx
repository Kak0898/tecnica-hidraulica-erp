...
import { BadgeDollarSign, Calculator, CalendarRange, ExternalLink, FileCheck2, LoaderCircle, RefreshCw, Save, Trash2, UploadCloud, UsersRound } from 'lucide-react'
...
export function ComprobantesComisiones() {
  ...
  return <div className="space-y-6">
    ...
    <Card>
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="rounded-xl bg-emerald-100 p-3 text-emerald-700"><CalendarRange size={22} /></div><div><h3 className="text-xl font-black">Calculadora mensual por vendedor</h3><p className="mt-1 text-sm text-slate-500">Sueldo base + arriendos + trabajos hidráulicos + ventas de apiladores.</p></div></div><button type="button" onClick={recalculateMonth} disabled={recalculating || !calculatorSellerId} className="inline-flex items-center justify-center gap-2 rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-2.5 text-sm font-black text-emerald-800 disabled:opacity-50"><RefreshCw size={17} className={recalculating ? 'animate-spin' : ''} />{recalculating ? 'Recalculando...' : 'Recalcular mes'}</button></div>
